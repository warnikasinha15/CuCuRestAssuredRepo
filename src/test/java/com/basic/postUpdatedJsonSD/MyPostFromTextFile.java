...
public class MyPostFromTextFile {
	...
	public void user_hit_the_post_request_with_json_data(String username) throws IOException{
		...
		//Read data from file
		String data = ""; 
	    data = new String(Files.readAllBytes(Paths.get("C:\\Users\\warni\\CucumberRestAssured\\src\\test\\java\\com\\basic\\postUpdatedJsonSD\\user.json"))); 
	    System.out.println("json in String ==== " +data);
	    
	    //Update the data
	    JSONObject object=new JSONObject(data);//json object is a class which is going except data in string format/constructor
	    object.put("name", username);//username from feature file
